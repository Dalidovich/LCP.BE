...
public class WatchSegmentNormalizerTests
{
    ...
    {
        Assert.Empty(WatchSegmentNormalizer.Normalize([]));
    }
    ...
    {
        Assert.Empty(WatchSegmentNormalizer.Normalize([new WatchSegmentRequest(30, 4.6)]));
    }

    [Fact]
    public void Normalize_RoundsStartAndDurationToWholeSeconds()
    {
        var segment = Assert.Single(WatchSegmentNormalizer.Normalize([new WatchSegmentRequest(21.4, 10.6)]));
    ...
    {
        var segment = Assert.Single(WatchSegmentNormalizer.Normalize([new WatchSegmentRequest(20.5, 10.5)]));
